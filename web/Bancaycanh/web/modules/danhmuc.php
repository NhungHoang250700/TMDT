...
<head>
  ...
  <meta charset="utf-8">
  <link rel="stylesheet" href="css/danhmuc.css" class="css">
...
<div class="container">
  <div class="row">
    ...
    $sql_select_product = mysqli_query($con,"SELECT A.*, B.* FROM products A, productlines B WHERE A.productLineCode=B.productLineCode AND A.productLineCode='$_GET[id]' ORDER BY productCode DESC LIMIT $sum_product_page OFFSET $offset");
    $totalRecord=mysqli_query($con,"SELECT * FROM products  WHERE productLineCode='$_GET[id]'");
    ...
    <div class="col-12 my-4 text-uppercase font13 cl_nau" id="breadcrumbs"><span><span>
      ...
      <span class="breadcrumb_last" aria-current="page"><?php echo $row_product['productLineName'] ?></span></span></span>
    </div>
    <div class="col-sm-9">
      <?php 
        $sql_post= mysqli_query($con, "SELECT * FROM newpost WHERE productLineCode='$_GET[id]' LIMIT 1");
        while ($row_post = mysqli_fetch_array($sql_post)) {
        ?>
        <h3><?php echo $row_post['name_post']?></h3>
        <div class="content_pd">
          <?php echo $row_post['content_post']?>
        </div>
        <?php
        }
        ?>
        <div class="product_new">
          <div class="pd-new">
            ...
            <ul class="pagination">
              ...
              ?>
              <li class="page-item">
                <a class="page-link" href="?web_moc=Danhmucsanpham&id=<?php echo $_GET['id']?>&per-page=<?php echo $sum_product_page?>&page=<?php echo $fist_page?>">First</a>
              </li>
              <?php
                }if($current_page > 1){
              ...
              ?>
              <li class="page-item">
                <a class="page-link" href="?web_moc=Danhmucsanpham&id=<?php echo $_GET['id']?>&per-page=<?php echo $sum_product_page?>&page=<?php echo $prev_page?>"><span aria-hidden="true">&laquo;</span></a>
              </li>
              <?php
              }
              ?>
              ...
              ?>
              <li class="page-item">
                <a class="page-link" href="?web_moc=Danhmucsanpham&id=<?php echo $_GET['id']?>&per-page=<?php echo $sum_product_page?>&page=<?php echo $num?>"><?php echo $num?></a>
              </li>
              <?php
                }  
              }else{
              ...
              <?php
                ...
                ?>
              <li class="page-item">
                <a class="page-link" href="?web_moc=Danhmucsanpham&id=<?php echo $_GET['id']?>&per-page=<?php echo $sum_product_page?>&page=<?php echo $next_page?>">Next</a>
              </li>
              <?php
              ...
              ?>
              <li class="page-item">
                <a class="page-link" href="?web_moc=Danhmucsanpham&id=<?php echo $_GET['id']?>&per-page=<?php echo $sum_product_page?>&page=<?php echo $end_page?>"><span aria-hidden="true">&raquo;</span></a>
              </li>
              <?php
              }
              ?>
